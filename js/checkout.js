    ...
    if (cart === null) {
      ...
      Object.values(cart).map((item, index) => {
        ...
        result += `
            ...
            <td data-label="Sub Total" colspan="2">${
  ...
  for (let i = 0; i < removeBtn.length; i++) {
    ...
    button.addEventListener("click", () => {
      ...
      localStorage.setItem(
        "cartNumbers",
        cartNumber - cartItems[productName].inCart
      );
      localStorage.setItem(
        "totalCost",
        totalCost - cartItems[productName].price * cartItems[productName].inCart
      );
...
proceedCheckout.addEventListener("click", (e) => {
  ...
  } else if (totalCost + 200 < 300) {
    swal({
      title: "Order Must Be Greater Than Rs. 300/-",
      icon: "info",
    });
  } else {
