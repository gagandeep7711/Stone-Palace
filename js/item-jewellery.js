...
async function getProducts() {
  try {
    ...
    for (let k = 0; k < view.length; k++) {
      view[k].addEventListener("click", () => {
        let products = data.JewelleryProduct;
        let selectItem = { ...products[k] };
        localStorage.setItem("selectedProduct", JSON.stringify(selectItem));
      });
    }
...
function displayProduct(products) {
  ...
  let result = "";
  products.JewelleryProduct.forEach((item) => {
    result += `
            ...
            <div class="product-desc">
                ...
                <div class="product-btn-container">
                    ...
                    <a href="${item.view}" class="product-btn view" id ="view" data-id=${item.id}><i class="fas fa-eye"></i> View</a>
                </div>
